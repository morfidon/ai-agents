...
Goal:Identify all code segments that could crash or throw runtime errors by static inspection only.Do not execute, patch, or modify any code. Only list potential risks.
...
Expected Output Format:Readable, organized by file and function.Example:
...
Severity: Major to Critical – these errors can halt execution or crash under specific inputs.Confidence: High – pattern-based, reliably detected by static rules.
