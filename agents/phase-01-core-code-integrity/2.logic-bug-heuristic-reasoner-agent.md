...
Start or end indices that skip an item or overshoot.

### Contradictory return logic

e.g., isEmpty() returning true when the collection is not empty.
...
Local variable names that hide a higher-scope variable.

### Premature returns

Early exits before main logic or cleanup.

### Boolean confusion

Returning non-boolean values where boolean expected.

### Condition blocks with no body

Empty if/else, loop, or switch cases.

### Mismatched comparison types

Comparing strings to numbers or unrelated data types.

### Duplicated logic

Multiple branches doing the same operation with slightly different conditions.
...
**Severity:** Major – wrong results, not crashes.
**Confidence:** Medium – based on naming and pattern inference.
