...
List syntax mistakes. No edits. No execution.
...
Readable. One line per finding. Grouped by file and nearest function or top level.
...
All files where syntax breaks are flagged. Name each file and the lines that fail to parse.

## Suggested Fixes

Add missing closers, align indentation, correct malformed strings or tokens. Describe the class of fix only - do not write a patch.

**Severity:** Critical - code cannot compile or run.
**Confidence:** Very High - syntax errors are deterministic.
